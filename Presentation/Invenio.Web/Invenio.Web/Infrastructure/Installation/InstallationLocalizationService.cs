...
namespace Invenio.Web.Infrastructure.Installation
{
    /// <summary>
                ...
                foreach (var filePath in Directory.EnumerateFiles(CommonHelper.MapPath("~/App_Data/Localization/Installation/"), "*.xml"))
                {
                    ...
                    foreach (XmlNode resNode in xmlDocument.SelectNodes(@"//Language/LocaleResource"))
                    {
                        ...
                        if (resNameAttribute == null)
                            throw new InvenioException("All installation resources must have an attribute Name=\"Value\".");
                        var resourceName = resNameAttribute.Value.Trim();
                        if (string.IsNullOrEmpty(resourceName))
                            throw new InvenioException("All installation resource attributes 'Name' must have a value.'");

                        if (resValueNode == null)
                            throw new InvenioException("All installation resources must have an element \"Value\".");
