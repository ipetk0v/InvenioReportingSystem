...
namespace Invenio.Web.Framework.Validators
{
    public abstract class BaseNopValidator<T> : AbstractValidator<T> where T : class
    {
        ...
        protected virtual void SetDecimalMaxValue<TObject>(IDbContext dbContext)
        {
            ...
            foreach (var expr in expression)
            {
                RuleFor(expr.Value).IsDecimal(maxValues[expr.Key]).WithMessage(string.Format(localizationService.GetResource("Invenio.Web.Framework.Validators.MaxDecimal"), maxValues[expr.Key] - 1));
            }
        }
    }
}